...
# Инициализация узла Nym и настройка брандмауэра
# Убираем аргумент --host, так как он, похоже, больше не поддерживается
nym-mixnode init --id $node_id
...
if service nym-mixnode status | grep -q "active (running)"; then
  echo -e "Ваш узел Nym \e[32mустановлен и работает\e[39m!"
  echo -e "Вы можете проверить состояние узла с помощью команды \e[7mservice nym-mixnode status\e[0m"
